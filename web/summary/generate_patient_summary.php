 ...
 *
 * Brief: This PHP script file solves the patient summary screen generation.
 *
 */
...
echo 'Set available number of locations:';
...
include ('patient_summary_aliases.php');
include ('patient_summary_datasource.php');

// 1st: get number of locations configured
// 2nd: generate base widget (int)(#ofLocations / 2) times
// 3nd: generate widget #ofLocations (int)(#ofLocations / 2) times
// 4th: generate states (int)(#ofLocations / 2) times with (#ofLocations % 2) widgets per each
...
if (isset($_GET['generatePatientSummary']))
{
  ...
  $numberOfLocations = $_GET['generatePatientSummary'];
  ...
  // a variable for each aliases
  for ($x = 1; $x <= $numberOfLocations; ++$x)
  {
    $copyAliasesTemplate = $summaryAliases;
    $XXX_ALIAS_NAME_XXX[$x] = "MVDevice" . str_pad($x, 3, "0", STR_PAD_LEFT);
    $XXX_ALIAS_ID_XXX[$x] = uniqidReal(32);
    $copyAliasesTemplate = str_replace("XXX_ALIAS_ID_XXX", $XXX_ALIAS_ID_XXX[$x], $copyAliasesTemplate);
    $copyAliasesTemplate = str_replace("XXX_ALIAS_NAME_XXX", $XXX_ALIAS_NAME_XXX[$x], $copyAliasesTemplate);
    if ($x != 1)
    {
      $allSummaryAliases .= ",";
      $allSummaryAliases .= $copyAliasesTemplate;
    }
    else
    {
      $allSummaryAliases .= $copyAliasesTemplate;
  ...
  // generate a datasource for each
  for ($x = 1; $x <= $numberOfLocations; ++$x)
  {
    $XXX_DATASOURCES_XXX[$x] = $datasourceTemplate;
    $XXX_DATASOURCES_XXX[$x] = str_replace("XXX_ENTITY_ALIAS_ID_XXX", $XXX_ALIAS_ID_XXX[$x], $XXX_DATASOURCES_XXX[$x]);
    if ($x != 1)
    {
      $allDatasources .= ",";
      $allDatasources .= $XXX_DATASOURCES_XXX[$x];
    }
    else
    {
      $allDatasources .= $XXX_DATASOURCES_XXX[$x];
...
echo '<br>'.'Num of locations: ' . $numberOfLocations . '<br>';

echo '<br>'.count($XXX_DATASOURCES_XXX).'<br>';
  ...
  for ($x = 1; $x <= $numberOfLocations; ++$x)
  {
    ...
    if ($cmdtuples == "")
    {
      ...
      echo '<br>';
      // TODO: CREATE MISSING DASHBOARD HERE
      $url = 'http://192.168.0.222/dashboards/add.php?newDashboard=MVDevice' . $x;
      $contents = file_get_contents($url);
      echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@" . substr($contents, 0, 200) . "#################################";
      //echo $contents;
    }
    ...
    $copyWidgetTemplate = str_replace("XXX_DATASOURCES_XXX", $XXX_DATASOURCES_XXX[$x], $copyWidgetTemplate);
  ...
  for ($i = 1; $i < ($numberOfLocations / 32) + 1; ++$i)
  {
    ...
    for ($x = 1; $x <= $numberOfLocations % 33; ++$x)
    {
  ...
  $copySummaryTemplate = str_replace("XXX_ALIASES_XXX", $allSummaryAliases, $copySummaryTemplate);
...
echo '<br><br>';
echo '<input type="text" id="numOfLocations" name="numOfLocations"><br><br><br><br>';
echo '<input id="btn" type="button" value="Generate patient summary dashboard" style="background-color:white" onclick="sendAddNewDashboardRequest()"/>';
...
<script>
function sendAddNewDashboardRequest()
{
  window.location.replace('http://192.168.0.222/summary/generate_patient_summary.php?generatePatientSummary=' + document.getElementById("numOfLocations").value);
